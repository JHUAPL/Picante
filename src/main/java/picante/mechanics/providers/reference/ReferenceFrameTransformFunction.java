 ...
 */
class ReferenceFrameTransformFunction<F extends FrameTransformFunction>
   ...
   * construction time.
   */
  final ChainLinkEngine<FrameID, F, FrameCodeProvider<F>> engine;

  /**
   * A linked list that is utilized by this routine to capture the connection functions that link
   * the from and to ID codes at the desired time.
   */
  final List<F> linkBuffer = new LinkedList<F>();

  /**
   * Frame transform utilized as a local buffer to store intermediate results.
   */
  private final RotationMatrixIJK transform = new RotationMatrixIJK();
   ...
   */
  ReferenceFrameTransformFunction(FrameID fromID, FrameID toID,
      ChainLinkEngine<FrameID, F, FrameCodeProvider<F>> engine) {
    super();
    this.fromID = fromID;
    this.toID = toID;
    this.engine = engine;
