...
public class DynamicTrueEquatorFrameFunction implements DynamicFrameFunction {
  private final int fromID;
  private final int toID;
  private boolean defined = false;

  public DynamicTrueEquatorFrameFunction(int fromID, int toID) {
    super();
  ...
  public RotationMatrixIJK getTransform(double time, RotationMatrixIJK buffer) {
    ...
    StateTransform nutationST = ida2st(Spicelib.ZZENUT80(time));
    StateTransform trueEqST = StateTransform.mxm(nutationST, precessionST, new StateTransform());
    return buffer.setTo(trueEqST.createInverse().getRotation());
  }
  ...
  public StateTransform getStateTransform(double time, StateTransform buffer) {
    ...
    StateTransform nutationST = ida2st(Spicelib.ZZENUT80(time));
    StateTransform trueEqST = StateTransform.mxm(nutationST, precessionST, new StateTransform());
    return buffer.setTo(trueEqST.createInverse());
  }
